...
func TestNewDefaultSchedulerMonitor(t *testing.T) {
	...
	t.Run("creates monitor with default settings", func(t *testing.T) {
		...
		assert.NotNil(t, monitor.jobChan, "job channel should be initialized")
		assert.NotNil(t, monitor.jobRecord, "job record map should be initialized")
...
func TestIncrementJob(t *testing.T) {
	...
	t.Run("increments job count for new job", func(t *testing.T) {
		...
		assert.Equal(t, 1, monitor.counter[jobName], "job count should be 1 after first increment")
	})

	t.Run("increments job count for existing job", func(t *testing.T) {
		t.Parallel()
		...
		assert.Equal(t, 3, monitor.counter[jobName], "job count should be 3 after three increments")
	})
...
func TestRecordJobTiming(t *testing.T) {
	...
	t.Run("records timing for new job", func(t *testing.T) {
		...
		require.Contains(t, monitor.time, jobName, "job should have timing records")
		assert.Len(t, monitor.time[jobName], 1, "should have one timing record")
	})

	t.Run("records multiple timings for same job", func(t *testing.T) {
		t.Parallel()

		monitor := newDefaultSchedulerMonitor()
		jobID := uuid.New()
		jobName := "multi-timing-job"
		tags := []string{"multi"}
...
//
// Test scenarios:
// - Happy path: record job timing with status
...
func TestRecordJobTimingWithStatus(t *testing.T) {
	...
	t.Run("records successful job execution", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("records failed job execution with error", func(t *testing.T) {
		t.Parallel()
...
func TestUpdateJobEvents(t *testing.T) {
	...
	t.Run("creates new job record", func(t *testing.T) {
		t.Parallel()
		...
		assert.True(t, ok, "job record should exist")
		assert.Equal(t, jobID.String(), spec.JobSpec.JobID, "job ID should match")
		assert.Equal(t, jobName, spec.JobSpec.JobName, "job name should match")
...
//
// Test scenarios:
// - Happy path: get job events
// - Boundary conditions: no events, empty job record
// - Exception cases: non-existent job
func TestGetJobEvents(t *testing.T) {
	...
	t.Run("returns nil for non-existent job", func(t *testing.T) {
		...
		assert.Nil(t, events, "should return nil for non-existent job")
	})
	...
	t.Run("returns nil for empty monitor", func(t *testing.T) {
		...
		assert.Nil(t, events, "should return nil when no job records exist")
	})

	t.Run("returns all events for existing job", func(t *testing.T) {
		t.Parallel()

		monitor := newDefaultSchedulerMonitor(WithMaxRecords(10))
		jobID := uuid.New()
		jobName := "get-events-job"
		tags := []string{"get"}
...
func TestWatch(t *testing.T) {
	...
	t.Run("returns job channel", func(t *testing.T) {
		t.Parallel()
...
func TestMonitorJobSpec(t *testing.T) {
	...
	t.Run("GetJobID returns correct job ID", func(t *testing.T) {
		...
		assert.Equal(t, "test-job-id-123", spec.GetJobID(), "should return correct job ID")
	})
	...
	t.Run("GetJobName returns correct job name", func(t *testing.T) {
		...
		assert.Equal(t, "test-job-name", spec.GetJobName(), "should return correct job name")
	})
...
//
// Test scenarios:
// - Happy path: get retry history for a job
...
func TestGetRetryHistory(t *testing.T) {
	...
	t.Run("returns nil for non-existent job", func(t *testing.T) {
		...
		monitor := newDefaultSchedulerMonitor()
		nonExistentJobID := "non-existent-job"

		history := monitor.GetRetryHistory(nonExistentJobID)
		assert.Nil(t, history, "should return nil for non-existent job")
	})

	t.Run("returns empty history for job with no retries", func(t *testing.T) {
		t.Parallel()
		...
		assert.Nil(t, history, "should return nil for job with no retry history")
	})

	t.Run("returns retry history for job with retries", func(t *testing.T) {
		t.Parallel()
