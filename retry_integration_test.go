// Package chrono provides integration tests for retry mechanism.
// 包 chrono 提供重试机制的集成测试。
package chrono
...
func TestSchedulerMonitorRetryMethods(t *testing.T) {
	...
	t.Run("GetRetryHistory returns nil for non-existent job", func(t *testing.T) {
		t.Parallel()

		history := monitor.GetRetryHistory("non-existent-job")
