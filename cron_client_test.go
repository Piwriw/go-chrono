...
func TestCronJobClientNilScheduler(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Add with nil scheduler returns ErrScheduleNil",
			...
			wantErr: ErrScheduleNil,
		},
		{
			name: "BatchAdd with nil scheduler returns ErrScheduleNil",
			setupFunc: func(c *CronJobClient) {
				c.scheduler = nil
			},
			testFunc: func(c *CronJobClient) error {
		...
		{
			name: "Remove with nil scheduler returns ErrScheduleNil",
			setupFunc: func(c *CronJobClient) {
				c.scheduler = nil
			},
			testFunc: func(c *CronJobClient) error {
		...
		{
			name: "Get with nil scheduler returns ErrScheduleNil",
			setupFunc: func(c *CronJobClient) {
				c.scheduler = nil
			},
			testFunc: func(c *CronJobClient) error {
...
// TestCronJobClientNilJob tests client behavior with nil job.
...
func TestCronJobClientNilJob(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Add with nil job returns ErrCronJobNil",
			setupFunc: func(c *CronJobClient) {
				c.job = nil
			},
			testFunc: func(c *CronJobClient) error {
			...
			wantErr: ErrCronJobNil,
		},
		{
			name: "Remove with nil job returns ErrCronJobNil",
			setupFunc: func(c *CronJobClient) {
				c.job = nil
			},
			testFunc: func(c *CronJobClient) error {
		...
		{
			name: "Get with nil job returns ErrCronJobNil",
			setupFunc: func(c *CronJobClient) {
				c.job = nil
			},
			testFunc: func(c *CronJobClient) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			scheduler, _ := NewScheduler(nil, nil)
			client := &CronJobClient{
				scheduler: scheduler,
			}
			if tt.setupFunc != nil {
...
func TestCronJobClientMethodChaining(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			testFunc: func() CronJobClientInterface {
				return client.Watch(func(event JobWatchInterface) {})
			},
		},
		{
...
// TestCronJobClientMethodsWithNilJob tests methods when job is nil.
...
func TestCronJobClientMethodsWithNilJob(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "CronExpr with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.CronExpr("* * * * *")
			},
		},
		{
			name: "Alias with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.Alias("test-alias")
			},
		},
		{
			name: "Name with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.Name("test-name")
			},
		},
		{
			name: "Tags with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.Tags("tag1")
			},
		},
		{
			name: "Task with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.Task(func() {})
			},
		},
		{
			name: "Watch with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.Watch(func(event JobWatchInterface) {})
			},
		},
		{
			name: "DefaultHooks with nil job returns client",
			testFunc: func() CronJobClientInterface {
				return client.DefaultHooks()
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.testFunc()
			if result == nil {
				t.Errorf("Method should return client even with nil job")
			}
		})
	}
}
...
func TestCronJobClientRemovePriority(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			alias:   "test-alias",
			jobName: "test-name",
			wantErr: ErrJobNotFound, // Job doesn't exist / 任务不存在
		},
		{
			...
			alias:   "test-alias",
			jobName: "test-name",
			wantErr: ErrFoundAlias, // Alias doesn't exist / 别名不存在
		},
		{
			...
			alias:   "",
			jobName: "test-name",
			wantErr: ErrJobNotFound, // Name doesn't exist / 名称不存在
		},
		{
			name:    "Remove with no identifier returns ErrJobNotFound",
			jobID:   "",
			alias:   "",
			jobName: "",
...
func TestCronJobClientGetPriority(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "Get with no identifier returns ErrJobNotFound",
			jobID:   "",
			alias:   "",
			jobName: "",
			wantErr: ErrJobNotFound,
...
func TestCronJobClientBatchAdd(t *testing.T) {
	...
	t.Run("BatchAdd with nil scheduler returns error", func(t *testing.T) {
		...
		_, err := client.BatchAdd(&CronJob{})
		if !errors.Is(err, ErrScheduleNil) {
			t.Errorf("BatchAdd() error = %v, wantErr %v", err, ErrScheduleNil)
		}
	})
}
