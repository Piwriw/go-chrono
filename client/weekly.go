...
type WeeklyJobClientInterface interface {
	...
	// Returns:
	//	WeeklyJobClientInterface - The client interface for method chaining / 客户端接口，支持链式调用
	Watch(watch func(event common.JobWatchInterface)) WeeklyJobClientInterface
	...
	// Add adds the configured weekly jobs to the scheduler.
	// 将配置的周任务添加到调度器。
	//
	// Returns:
	...
	BatchAdd(weeklyJobs ...*jobs.WeeklyJob) ([]gocron.Job, error)

	// Remove removes the weekly jobs from the scheduler.
	// 从调度器中删除周任务。
	//
	// Returns:
	...
	// Get retrieves the weekly jobs from the scheduler.
	// 从调度器中获取周任务。
	//
	// Returns:
...
//
//	WeeklyJobClientInterface - The client interface for method chaining / 客户端接口，支持链式调用
func (w *WeeklyJobClient) Watch(watch func(event common.JobWatchInterface)) WeeklyJobClientInterface {
	w.job.Watch(watch)
	return w
}
...
func (w *WeeklyJobClient) Add() (gocron.Job, error) {
	...
	// Check if jobs is nil
	// 检查任务是否为空
	if w.job == nil {
		return nil, common.ErrWeeklyJobNil
	}
...
//	[]gocron.Job - The list of added jobs / 添加的任务列表
//	error         - Error if jobs cannot be added / 如果无法添加任务的错误
func (w *WeeklyJobClient) BatchAdd(weeklyJobs ...*jobs.WeeklyJob) ([]gocron.Job, error) {
	// Check if scheduler is nil
	// 检查调度器是否为空
	if w.scheduler == nil {
		return nil, common.ErrScheduleNil
	...
	jobs := make([]any, len(weeklyJobs))
	for i, j := range weeklyJobs {
		jobs[i] = j
	}
	return w.scheduler.AddWeeklyJobs(jobs...)
}
...
func (w *WeeklyJobClient) WithRetry(maxRetries int, policy retry.RetryPolicy) WeeklyJobClientInterface {
	...
	if w.job.JobOptions == nil {
		w.job.JobOptions = common.NewJobOptions()
	}
	w.job.JobOptions.SetRetryConfig(&retry.RetryConfig{
...
func (w *WeeklyJobClient) WithRetryConfig(config *retry.RetryConfig) WeeklyJobClientInterface {
	...
	w.job.JobOptions.SetRetryConfig(config)
	return w
}
