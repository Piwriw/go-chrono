...
type CronJobClientInterface interface {
	// CronExpr sets the Linux cron expression for the jobs (required field).
	...
	// Returns:
	//	CronJobClientInterface - The client interface for method chaining / 客户端接口，支持链式调用
	Watch(watch func(event common.JobWatchInterface)) CronJobClientInterface
	...
	// Add adds the configured cron jobs to the scheduler.
	// 将配置的定时任务添加到调度器。
	//
	// Returns:
	...
	BatchAdd(cronJobs ...*jobs.CronJob) ([]gocron.Job, error)

	// Remove removes the cron jobs from the scheduler.
	// 从调度器中删除定时任务。
	//
	// Returns:
	...
	// Get retrieves the cron jobs from the scheduler.
	// 从调度器中获取定时任务。
	//
	// Returns:
...
//
// The client uses a mutex to ensure thread-safe access to the scheduler and jobs fields.
...
// CronExpr sets the Linux cron expression for the jobs.
...
//
//	CronJobClientInterface - The client interface for method chaining / 客户端接口，支持链式调用
func (c *CronJobClient) Watch(watch func(event common.JobWatchInterface)) CronJobClientInterface {
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.job == nil {
		return c
...
//	[]gocron.Job - The list of added jobs / 添加的任务列表
//	error         - Error if jobs cannot be added / 如果无法添加任务的错误
func (c *CronJobClient) BatchAdd(cronJobs ...*jobs.CronJob) ([]gocron.Job, error) {
	c.mu.RLock()
	defer c.mu.RUnlock()
	if c.scheduler == nil {
		return nil, common.ErrScheduleNil
	...
	jobs := make([]any, len(cronJobs))
	for i, j := range cronJobs {
		jobs[i] = j
	}
	return c.scheduler.AddCronJobs(jobs...)
}
...
// NewCronJobClient creates a new cron jobs client with the given scheduler.
// NewCronJobClient 创建一个新的定时任务客户端。
//
// Parameters:
//
//	scheduler - The scheduler instance / 调度器实例
//
// Returns:
//
//	CronJobClientInterface - The cron jobs client interface / 定时任务客户端接口
func NewCronJobClient(scheduler common.SchedulerInterface) CronJobClientInterface {
...
// NewCronJobClientWithJob creates a new cron jobs client with the given scheduler and jobs.
// NewCronJobClientWithJob 创建一个带有调度器和任务的定时任务客户端。
//
// Parameters:
//
//	scheduler - The scheduler instance / 调度器实例
//	jobs       - The cron jobs / 定时任务
//
// Returns:
//
//	CronJobClientInterface - The cron jobs client interface / 定时任务客户端接口
func NewCronJobClientWithJob(scheduler common.SchedulerInterface, job *jobs.CronJob) CronJobClientInterface {
	return &CronJobClient{
		scheduler: scheduler,
		job:       job,
	}
