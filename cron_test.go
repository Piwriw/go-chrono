...
func TestNewCronJob(t *testing.T) {
	...
	tests := []struct {
		...
		expectedType JobType // 预期任务类型 / Expected job type
		expectedExpr string  // 预期表达式 / Expected expression
	}{
		{
			name:         "create cron job with standard expression",
			expr:         "0 * * * *",
			expectedType: JobTypeCron,
			expectedExpr: "0 * * * *",
		},
		{
			name:         "create cron job with every minute",
			expr:         "* * * * *",
			expectedType: JobTypeCron,
			expectedExpr: "* * * * *",
		},
		{
			name:         "create cron job with complex expression",
			expr:         "0 9-17 * * 1-5",
			expectedType: JobTypeCron,
			expectedExpr: "0 9-17 * * 1-5",
		},
		{
			name:         "create cron job with empty expression",
			expr:         "",
			expectedType: JobTypeCron,
			expectedExpr: "",
		},
		{
			name:         "create cron job with seconds expression",
			expr:         "*/5 * * * * *",
			expectedType: JobTypeCron,
			expectedExpr: "*/5 * * * * *",
		},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute constructor
			// 执行构造函数
			job := NewCronJob(tt.expr)

			// Verify job properties
			// 验证任务属性
			assert.NotNil(t, job, "NewCronJob should return non-nil CronJob")
			assert.Equal(t, tt.expectedType, job.Type, "Job type should be JobTypeCron")
...
func TestCronJobCronExpr(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "set cron expression on new job",
			initialExpr:  "",
			newExpr:      "0 9 * * *",
			expectedExpr: "0 9 * * *",
		},
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create job and set expression
			// 创建任务并设置表达式
			job := NewCronJob(tt.initialExpr)
			result := job.CronExpr(tt.newExpr)

			// Verify method chaining and expression
			// 验证方法链式调用和表达式
			assert.Same(t, job, result, "CronExpr should return same job instance for chaining")
...
func TestCronJobAlias(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			alias:         "job-123",
			expectedAlias: "job-123",
		},
		{
			name:          "set alias with special characters",
			alias:         "job@v1.0",
			expectedAlias: "job@v1.0",
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用和别名
			assert.Same(t, job, result, "Alias should return same job instance for chaining")
...
//
// Test scenarios:
// - Happy path: set job ID and verify chaining
...
func TestCronJobJobID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "set simple job ID",
			jobID:      "job-001",
			expectedID: "job-001",
		},
		{
			name:       "set UUID as job ID",
			jobID:      uuid.New().String(),
			expectedID: uuid.New().String(), // Will be different, just testing type
		},
		{
			name:       "set empty job ID",
			jobID:      "",
			expectedID: "",
		},
		{
			name:       "set numeric job ID",
			jobID:      "12345",
			expectedID: "12345",
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用和 ID
			assert.Same(t, job, result, "JobID should return same job instance for chaining")
			if tt.jobID != "" {
...
func TestCronJobNames(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用和名称
			assert.Same(t, job, result, "Names should return same job instance for chaining")
...
func TestCronJobTag(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用和标签
			assert.Same(t, job, result, "Tag should return same job instance for chaining")
...
func TestCronJobTask(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用
			assert.Same(t, job, result, "Task should return same job instance for chaining")

			// Verify error state
			// 验证错误状态
			if tt.wantErr {
				require.Error(t, job.err, "Job should have error set")
				assert.ErrorIs(t, job.err, tt.expectedError, "Error should match expected error")
			} else {
				assert.NoError(t, job.err, "Job should not have error")
			}
...
func TestCronJobWatch(t *testing.T) {
	...
	// 定义测试监听函数
	customWatch := func(event JobWatchInterface) {
		// Custom watch logic
	}

	tests := []struct {
		name      string                        // 测试用例名称 / Test case name
		watchFunc func(event JobWatchInterface) // 监听函数 / Watch function
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 验证方法链式调用和监听函数
			assert.Same(t, job, result, "Watch should return same job instance for chaining")
...
func TestCronJobDefaultHooks(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:              "add default hooks to new job",
			initialHookCount:  0,
			callDefaultHooks:  true,
			expectedHookCount: 6, // 6 default hooks
		},
		{
			name:              "add default hooks to job with existing hooks",
			initialHookCount:  2,
			callDefaultHooks:  true,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create job and add initial hooks
			// 创建任务并添加初始钩子
			job := NewCronJob("* * * * *")

			for i := 0; i < tt.initialHookCount; i++ {
				job.addHooks(gocron.BeforeJobRuns(func(jobID uuid.UUID, jobName string) {
					// Custom hook
				}))
			}
			...
			if tt.callDefaultHooks {
				...
				// Verify method chaining
				// 验证方法链式调用
				assert.Same(t, job, result, "DefaultHooks should return same job instance for chaining")
			}
...
func TestCronJobBeforeJobRuns(t *testing.T) {
	...
	hookFunc := func(jobID uuid.UUID, jobName string) {
		// Before job hook logic
	}

	result := job.BeforeJobRuns(hookFunc)

	assert.Same(t, job, result, "BeforeJobRuns should return same job instance")
...
func TestCronJobBeforeJobRunsSkipIfBeforeFuncErrors(t *testing.T) {
	...
	assert.Same(t, job, result, "BeforeJobRunsSkipIfBeforeFuncErrors should return same job instance")
	assert.Len(t, job.Hooks, 1, "Should have one hook")
}
...
func TestCronJobAfterJobRuns(t *testing.T) {
	...
	assert.Same(t, job, result, "AfterJobRuns should return same job instance")
	assert.Len(t, job.Hooks, 1, "Should have one hook")
}
...
func TestCronJobAfterJobRunsWithError(t *testing.T) {
	...
	assert.Same(t, job, result, "AfterJobRunsWithError should return same job instance")
	assert.Len(t, job.Hooks, 1, "Should have one hook")
}
...
func TestCronJobAfterJobRunsWithPanic(t *testing.T) {
	...
	assert.Same(t, job, result, "AfterJobRunsWithPanic should return same job instance")
	assert.Len(t, job.Hooks, 1, "Should have one hook")
}
...
func TestCronJobAfterLockError(t *testing.T) {
	...
	assert.Same(t, job, result, "AfterLockError should return same job instance")
	assert.Len(t, job.Hooks, 1, "Should have one hook")
}
...
func TestWeekTimeToCronUtil(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute conversion
			// 执行转换
			result := WeekTimeToCron(tt.time, tt.weekday)
...
func TestMonthTimeToCronUtil(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute conversion
			// 执行转换
			result := MonthTimeToCron(tt.time, tt.month)
...
func TestCronJobChaining(t *testing.T) {
	...
	assert.Equal(t, "daily-job", job.Ali, "Alias should be set")
	assert.Equal(t, "job-001", job.ID, "JobID should be set")
...
func TestTimeTypeConstants(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			constant: DayTimeType,
			expected: "%d %d * * *",
		},
		{
			name:     "WeekTimeType constant",
			constant: WeekTimeType,
			expected: "%d %d * * %d",
		},
		{
			name:     "MonthTimeType constant",
			constant: MonthTimeType,
			expected: "%d %d * %d *",
		},
	}
...
func ExampleCronJobChaining() {
	// Create and configure a cron job using method chaining
...
func ExampleWeekTimeToCron() {
	...
	t := time.Date(2024, 1, 1, 9, 30, 0, 0, time.UTC)
	expr := WeekTimeToCron(t, time.Monday)
	println(expr)
	// Output: 30 9 * * 1
}
...
func ExampleMonthTimeToCron() {
	...
	t := time.Date(2024, 1, 1, 9, 30, 0, 0, time.UTC)
	expr := MonthTimeToCron(t, time.January)
	println(expr)
	// Output: 30 9 * 1 *
}
