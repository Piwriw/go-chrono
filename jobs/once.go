...
// OnceJob represents a jobs that runs only once at specified times.
...
type OnceJob struct {
	...
	// Name 是任务的名称。
	Name string
	// WorkTime is the specific times when the jobs should run.
...
// Parameters:
//
//	workTimes - Variable number of time values when jobs should run / 任务应该运行的可变数量时间值
//
// Returns:
//
//	*OnceJob - The newly created one-time jobs / 新创建的一次性任务
...
//
//	*OnceJob - The one-time jobs for method chaining / 支持链式调用的一次性任务
func (c *OnceJob) Watch(watch func(event common.JobWatchInterface)) *OnceJob {
	c.WatchFunc = watch
	return c
}
