...
// MonthJob represents a jobs that runs on a monthly schedule.
...
type MonthJob struct {
	...
	// Name 是任务的名称。
	Name string
	// Interval is the interval in months between jobs runs.
	// Interval 是任务运行的月数间隔。
	Interval uint
	// DaysOfTheMonth are the days of the month to run the jobs.
	// DaysOfTheMonth 是任务每月运行的具体日期。
	DaysOfTheMonth gocron.DaysOfTheMonth
	// AtTimes are the specific times of day to run the jobs.
	// AtTimes 是任务每天运行的具体时间点。
	AtTimes gocron.AtTimes
	// Tag are the tags for the jobs.
	// Tag 是任务的标签。
	Tag []string
...
// Parameters:
//
//	interval - The interval in months between jobs runs / 任务运行之间的月数间隔
//	days    - The days of the month to run the jobs / 任务运行的月份日期
//	atTime  - The specific times to run the jobs / 任务运行的具体时间
//
// Returns:
//
//	*MonthJob - The newly created monthly jobs / 新创建的月度任务
...
// Returns:
//
//	*MonthJob - The monthly jobs for method chaining / 支持链式调用的月度任务
...
//
//	*MonthJob - The monthly jobs for method chaining / 支持链式调用的月度任务
func (c *MonthJob) Watch(watch func(event common.JobWatchInterface)) *MonthJob {
	c.WatchFunc = watch
	return c
}
...
func (j *MonthJob) GetError() error {
	return j.err
}
