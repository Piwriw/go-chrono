...
// TestCallJobFunc tests the callJobFunc utility function.
// TestCallJobFunc 测试 callJobFunc 工具函数。
//
// Test scenarios:
...
func TestCallJobFunc(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// 执行函数
			err := callJobFunc(tt.jobFunc, tt.params...)

			// Verify results
			// 验证结果
			if tt.wantErr {
				require.Error(t, err, "callJobFunc should return an error")
				if tt.errMsg != "" {
					assert.Contains(t, err.Error(), tt.errMsg, "error message should contain expected substring")
				}
			} else {
				assert.NoError(t, err, "callJobFunc should not return an error")
			}
		})
	}
}

// BenchmarkCallJobFunc benchmarks the callJobFunc function.
// BenchmarkCallJobFunc 对 callJobFunc 函数进行基准测试。
func BenchmarkCallJobFunc(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = callJobFunc(fn, params...)
	}
}

// ExampleCallJobFunc demonstrates the usage of callJobFunc.
// ExampleCallJobFunc 展示 callJobFunc 的使用方法。
func ExampleCallJobFunc() {
	...
	// 使用参数调用函数
	err := callJobFunc(task, "example", 10)
	if err != nil {
...
// TestValidateURLAddr tests the validateURLAddr utility function.
// TestValidateURLAddr 测试 validateURLAddr 工具函数。
//
// Test scenarios:
...
func TestValidateURLAddr(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			// Execute validation
			// 执行验证
			err := validateURLAddr(tt.addr)

			// Verify results
			// 验证结果
			if tt.wantErr {
				require.Error(t, err, "validateURLAddr should return an error")
				if tt.errMsg != "" {
					assert.Contains(t, err.Error(), tt.errMsg, "error message should contain expected substring")
				}
			} else {
				assert.NoError(t, err, "validateURLAddr should not return an error")
			}
		})
	}
}

// BenchmarkValidateURLAddr benchmarks the validateURLAddr function.
// BenchmarkValidateURLAddr 对 validateURLAddr 函数进行基准测试。
func BenchmarkValidateURLAddr(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = validateURLAddr(validAddr)
	}
}

// ExampleValidateURLAddr demonstrates the usage of validateURLAddr.
// ExampleValidateURLAddr 展示 validateURLAddr 的使用方法。
func ExampleValidateURLAddr() {
	// Valid addresses
	// 有效地址
	err := validateURLAddr("localhost:8080")
	fmt.Println("localhost:8080:", err == nil)

	err = validateURLAddr("192.168.1.1:9090")
	fmt.Println("192.168.1.1:9090:", err == nil)

	// Invalid address
	// 无效地址
	err = validateURLAddr("example.com")
