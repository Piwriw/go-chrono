...
// DailyJobClientInterface defines the interface for configuring and managing daily jobs.
// 定义用于配置和管理每日任务的接口。
type DailyJobClientInterface interface {
	// AtDayTime sets the job to run at a specific time every day.
	// 设置任务在每天的指定时间运行。
	//
	// Parameters:
	...
	// Add adds the configured daily job to the scheduler.
	// 将配置的每日任务添加到调度器。
	//
	// Returns:
	...
	// Remove removes the daily job from the scheduler.
	// 从调度器中删除每日任务。
	//
	// Returns:
	//	error - Error if the job cannot be removed / 如果无法删除任务的错误
	Remove() error
	// Get retrieves the daily job from the scheduler.
	// 从调度器中获取每日任务。
	//
	// Returns:
